...
// todo: update가 완료된 컴포넌트 구별 변수 추가? _isUpdated
// todo: tab, dropdown 등 특정 요소를 숨겼다가 노출하는 ui에서 업데이트는 1회만 적용
// todo: date, month, time > format
// todo: mm.datepicker
// todo: mm.sortable
...
var __mm__ = {
	...
	__: {
		...
		_isStage: false,// ? :boolean - 스페이지 활성화
		_publishHomeUrl: '_stage.html',// ? :string - 퍼블 스테이지
		_realHomeUrl: '',// ? :string - 개발 홈
...
mm._isExternal = mm.is.layout('external');// 외부
// mm._isPrint = mm.is.layout('print');// 프린트
mm._isMain = mm.is.layout('main');// 메인
mm._isSide = mm.is.layout('side');// 사이드
mm._isSearch = mm.is.layout('search');// 검색
mm._isProduct = mm.is.layout('product');// 상품상세
...
mm.event.type.set('main_go');// 메인으로 이동(슬라이드 변경을 위해 사용)
mm.event.type.set('stage_add');// 생성된 팝업 요소를 스테이지에 추가
mm.event.type.set('stage_remove');// 뒤로가면 현재페이지(팝업) 이후 저장된 요소 삭제
//> 이벤트 커스텀
...
mm.link = function (__url, __option) {
	...
	var option = mm.extend({
		...
		_isReloadStage: false,// ? :boolean - type이 home일 때 메인으로 이동 후 새로고침
		_isLinkStage: true,// ? :boolean - type이 link일 때 true(스테이지에서 실행 mm.popup.open), false(현재 창에서 실행 location.href)
		_historyDiff: 0,// ? :number - 페이지에서 location.href로 이동된 히스토리 수
		state: {},// ? :object - history.state에 추가로 저장할 값
		// 이 외 mm.scroll.to, mm.modal.open 등에서 사용하는 변수를 추가로 적용하여 사용
	}, __option);
	...
	switch (option._type) {
			...
			if (location.href.split('#')[0] !== __url.split('#')[0] && option._isLinkStage) {
				...
				if (option._isProduct === true && option.openEl) {
					...
					if ($prodImage) {
						...
						mm.find('.mm_popup-items', top.document)[0].append($cloneImage);// frame 내부에서 실행될 수 있어 최상단 mm_popup-items에 append

						var $cloneWrap = mm.element.wrap($cloneImage, 'div')[0];
						$cloneWrap.classList.add('m_product-clone');

						// 기존 페이지 새로고침 시 오류로 top에서 실행
				...
				// url이 같으면 리로드
				// if (location.href.includes(__url)) location.replace(location.href.split('#')[0]);// includes를 사용하면 order/main, order/ 와 같이 상위 링크가 /로 끝날 때 이슈
				if (location.href.split('#')[0] === __url.split('#')[0]) location.replace(location.href.split('#')[0]);// location.reload();
				// 현재 창에서 변경
				else {
					mm.history.replace({}, __url, { _isTop: false });// iframe 주소 변경
					mm.history.push({ _isNew: false }, __url);// 최상단 주소 변경
					// location.reload();// 미들웨어에서 새로고침 되면서 스테이지를 하나 더 불러오는 이슈로 replace로 변경
		...
		case 'home':
			...
			var _mainHash = __url.split('#')[1];// 메인 이동 시 노출할 슬라이드를 #해시로 연결
			if (_mainHash && _mainHash.length > 0) mm.storage.set('session', '_mainHash', _mainHash);

			// 메인 새로고침
			if (!state || state._pageIndex < 1 || (!session.pages[0]._pageUrl.includes(`/main`) && mm.storage.get('session', 'isCancelPopstate') !== true)) {
				// 현재 페이지가 메인이면 새로고침
				if (frameElement && frameElement.src.replace(location.origin, '') === mm._mainUrl) location.replace(location.href.split('#')[0]);
				// 메인이 아니면 스테이지로 이동 후 세션 새로 생성
				else {
					mm.history.replace(null, mm._homeUrl);// 스테이지로 이동
					top.location.reload();
				}
			}
			else {
				var _backCount = state._pageIndex + session.page.changes.length + option._historyDiff;
                if (session.pages[0]._pageUrl.includes(`utm_source=naver`)) _backCount--;

				session.history._isReloadStage = option._isReloadStage;
				session.page.changes = [];// 앞으로 가기 시 중복 적용 방지를 위해 초기화
...
mm.history = (function () {
	...
	return {
		...
		back: function (__step, __callback, __params) {
			...
			var state = base.state;
			// 첫페이지 경로가 메인(스테이지)이 아니거나 첫페이지가 아닐 때 팝업이 없으면 뒤로가기 클릭 시 메인(스테이지)으로 이동
...
mm.toast = (function () {
	...
	// $el['__mm.data__'].name에 저장할 기본 값
	var initial = {
		_classOn: '__toast-on',// ? :string
		_title: '접어놓기',// ? :string - on일 때 적용할 title 속성 값
		_defaultTitle: '펼쳐보기',// ? :string - 기본 title 속성 값
		_isReturnParams: true,// ? :boolean - onChange의 1번째 인자로 true/false 전달 여부
		onChange: null,// ? :function
		onChangeParams: [],// ? :array - onChange의 2번째 이후 아규먼트로 전달
		// 내부사용
	...
	// UI 고유 객체
	var base = {
		get _dataName() { return 'data-toast'; },// 데이타 속성 이름
		toggle: function (__$toast, __is) {

			// ? __$toast:element - 스위치 단일 요소
			// ? __is:boolean - 활성 여부
		...
		mm.delegate.on(document, mm.string.template('[${NAME}] .btn_toast', { NAME: base._dataName }), 'touchstart', function (__e) {
			...
			mm.event.on($toast, 'touchmove touchend', function timedealTouchInlineHandler(__e) {
				...
				if (__e.type === 'touchmove') {
					...
					var _touchTime = mm.time.stampEnd('toast') / 1000;// 빠른 드래그 확인
					if (_touchTime > 0.2) {
						gsap.to(data.__, {
...
mm.dropdown = (function () {
	...
	var base = {
		...
		toggle: function (__$ui, __is, __time) {
			...
			else if (mm._isFrame && !mm._isMain) mm.frameResize();// 컨텐츠 아이프레임 리사이즈
	...
	var base = {
		...
		update: function (__$element) {
			...
			mm.event.on($inner, 'touchstart', function carouselTouchStartInlineHandler(__e) {
				...
				// app_page-common에서 캐러셀 아이프레임 터치영역이 document일 때 터치 제외를 위해 사용
				// var $eventTarget = (__e.detail && __e.detail.target) ? __e.detail.target : __e.target;
				// var $parentUi = $eventTarget.closest(mm.selector([base._dataName, 'data-toast'], '[]'));
				// if ($parentUi && !$parentUi.matches(mm.selector(base._dataName, '[]'))) return;// 부모가 캐러셀이 아니면 리턴($parentUi가 없으면 iframe 내부로 판단)

				clearInterval(data.__.interval);
				mm.time.stamp('carousel');

				// 아이프레임 내부에서 터치 시 __e.detail.touches로 값 전달
	...
	var base = {
		...
		update: function (__$element) {
			...
			mm.event.on($inner, 'touchstart', function sliderTouchStartInlineHandler(__e) {
				...
				// app_page-common에서 슬라이더 아이프레임 터치영역이 document일 때 터치 제외를 위해 사용
				// var $eventTarget = (__e.detail && __e.detail.target) ? __e.detail.target : __e.target;
				// var $parentUi = $eventTarget.closest(mm.selector([base._dataName, 'data-toast'], '[]'));
				// if ($parentUi && !$parentUi.matches(mm.selector(base._dataName, '[]'))) return;// 부모가 슬라이더가 아니면 리턴($parentUi가 없으면 iframe 내부로 판단)
...
mm.form.text = (function () {
	...
	(function () {
		...
		mm.delegate.on(document, '.btn_text-clear', 'click', function (__e) {
			...
			if (!['date', 'month', 'time'].includes($text.type)) mm.focus.in($text);// 날짜관련 폼은 모바일 기본 창이 열려 제외
...
mm.popup = (function () {
	...
	var base = {
		$popup: null,
		_isReady: true,// ? :boolean - 팝업 준비 상태, 값이 false이면 이전 팝업 실행 중
		waits: [],// ? :array - 여러 팝업을 히스토리 등으로 빠르게 열 때 중복 방지를 위한 대기열
		get _classOn() { return '__popup-on'; },
		get _classOld() { return '__popup-old'; },// 이전 팝업
		//- 오프너(window)
		get opener() {
		...
		closeItem: function (__option) {
			...
			var option = mm.extend({
				_historyDiff: 0,// ? :number - 페이지에서 location.href로 이동된 히스토리 수
				_isHistoryBack: false,// ? :boolean - popstate 뒤로가기로 생성
				onClose: null,
				onCloseParams: [],
			}, __option);
			var state = mm.history.state;

			// 팝업 닫기 액션을 통일하기 위해 히스토리로 제어
			if (!option._isHistoryBack) {
				// 아이프레임 내부에서 location.href, submit 등으로 강제로 링크가 이동된 history 재조정
				var session = mm.history.session;
				var _backCount = state._keepIndex + session.page.changes.length + option._historyDiff + 1;

				mm.history.back(_backCount);
			}
			// 히스토리에서 실행 시 팝업 닫기 적용
			else {
				...
				// 페이지에 팝업 요소가 전혀 없으면 강제로 1초 후에 메인 노출
	...
	return {
		...
		open: function (__url, __option) {
			...
			if (!__url) return;
			if (frameElement) return mm.apply('mm.popup.open', top, arguments);// 최상위에서 실행

			if (!base._isReady) return base.waits.push([__url, __option]);// 대기열에 추가
			base._isReady = false;
			...
			var option = mm.extend({
				...
				focusEl: null,// ? :element - 팝업이 노출된 후 focusin 할 요소
				...
				_isCloseBefore: false,// ? :boolean - 열려있던 팝업 요소를 닫음(교체)
				_isHistoryBack: false,// ? :boolean - popstate 뒤로가기로 생성
				_isHistorySave: true,// ? :boolean - 히스토리에 저장(popstate에서 생성된 페이지는 false 저장 안함)
			...
			var $popupItem = mm.element.create(mm.string.template([
				...
				'	</button>',
				'	<iframe scrolling="no" onload="mm.popup.onload(this);"></iframe>',// #앵커 url 지원 안함
				'</div>',
			]))[0];

            if (mm.history.session.pages[0]._pageUrl.includes(`utm_source=naver`)) {
                $popupItem = mm.element.create(mm.string.template([
                    '<div class="mm_popup-item">',
                    '	<button type="button" class="btn_popup-close" onclick="mm.history.back();">',
                    '		<i class="mco_popup-close"></i>',
                    '		<b class="mm_ir-blind">팝업닫기</b>',
                    '	</button>',
                    '	<iframe scrolling="no" onload="mm.popup.onload(this);"></iframe>',// #앵커 url 지원 안함
			...
			mm.observer.on($popupItem, mm.event.type.frame_ready, function (__e) {
				...
				if ($items.length === 0 || option._isHistoryBack) $popupItem.classList.add('__popup-nomotion');// 첫 페이지와 뒤로가기로 생성된 페이지는 모션없음

				// 페이지 타입과 모션 설정
				var session = mm.history.session;
				var sessionPage = session.page;
				var $frameWindow = __e.detail.this;
				var _isTypeLink = sessionPage._pageType === 'keep';// ready 이벤트가 once로 한 번만 실행되어 keep 확인이 필요없지만 새로고침에서 비교
				...
				if (_isTypeLink) sessionPage._pageType = 'keep';// 모션 클래스 적용 후 keep으로 변경
				if ($popupItem.getAttribute('class').indexOf('__popup-motion-') < 0) $popupItem.classList.add('__popup-motion-left');// sub, search

				mm.history.session = session;// 히스토리와 세션에 저장
				if (sessionPage._pageType !== 'popup' || mm.find('.btn_popup-close', $iframe)[0]) mm.element.remove(mm.find('.btn_popup-close', $popupItem));// iframe 내부에 같은 버튼이 있으면 삭제

				mm.loading.hide();
				mm.delay.off('DELAY_LINK_MAIN');// 강제 이동 해제
				...
				mm.delay.on(function () {
					...
					if (option._isHistoryBack) base.closeItem({ _isHistoryBack: true });// 뒤로가기로 생성 시 덮고 있는 팝업 닫기

					var _isRemoveStage = (sessionPage._pageType === 'side') ? false : (option._isCloseBefore) ? true : !option._isHistoryBack;// 뒤로가기가 아닐 때 사이드는 삭제 별도 적용
			...
			// _isCloseBefore로 인해 딜레이 필요
			mm.delay.on(function () {

				// 히스토리에 추가
				if (option._isHistorySave) mm.history.push({ _isNew: true }, __url);

				mm.data.set($popupItem, 'popup', { initial: { openEl: option.openEl, focusEl: option.focusEl, _index: mm.history.state._pageIndex } });// _pageIndex와 비교를 위해 _index 추가
		...
		onload: function (__frameElement) {
			...
			// ? __frameElement:element - 아이프레임 단일 요소
			...
			// 내부 도메인
			try {
				// mm.link가 아닌 iframe 내부에서 직접 변경하면 history의 onpopstate에 의한 변경인지 링크 이동인지 확인이 안되어 외부도메인처럼 changes에서 관리
				_frameUrl = frameLocation.href;
				if (_frameUrl.length < 5 || _frameUrl === 'about:blank') _frameUrl = '';
			}
			// 외부도메인
			catch (__error) {
				_frameUrl = '/external/crossdomain/' + new Date().getTime();// 가상의 url을 생성하여 저장
			}
			_frameUrl = _frameUrl.replace(location.origin, '');

			// 세션 page.changes에 저장
			var session = mm.history.session;
			// if (_frameUrl.length > 0 && _frameUrl !== session.page._pageUrl) {// 이전 페이지 로드가 늦게되면 changes에 저장되는 이슈로 변경
...
mm.colorpicker = (function () {
	...
	var base = {
		...
		appendPicker: function () {
			...
			base.$picker = mm.element.create(mm.string.template([
				'<div class="mm__colorpicker">',
				'	<div class="mm__colorpicker-list">',
				'		<ul></ul>',
				'		<ul></ul>',
				'	</div>',
				'	<div class="mm__colorpicker-foot">',
			...
			var $colorLists = mm.find('.mm__colorpicker-list ul', base.$picker);
	...
	(function () {
		...
		mm.delegate.on(document, mm.string.template('[${UI}] .btn_picker', { UI: base._dataName }), 'click', function (__e) {
			...
			var $picker = mm.siblings(this, '.mm__colorpicker')[0];
	...
	return {
		...
		open: function (__$element) {
			...
			mm.event.on(document, 'click', function colorClickInlineHandler(__e) {
				...
				if (!__e.target.closest('.mm__colorpicker')) base.closePicker();
...
mm.pinchzoom = (function () {
	...
	var initial = {
		_scale: 1,// ? :number - 초기 scale 값
		_scaleMin: 1,// ? :number - 핀치 줌인 가능한 최소 배수
		_scaleMax: 3,// ? :number - 핀치 줌인 가능한 최대 배수
		_classWrapper: 'mm_pinchzoom-wrapper',
		padding: {// :object - 핀치 영역 상, 하단에 fixed 요소가 겹쳐있는 경우 해당 요소의 높이값 만큼 padding 값 추가
			_top: 0,// ? :number - 상단 여백
			_bottom: 0// ? :number - 하단 여백
		}
	};

	// UI 고유 객체
	var base = {
		get _dataName() { return 'data-pinchzoom'; },// 데이타 속성 이름
		// 핀치 터치 이벤트(모바일)
		onTouch: function (__element, __data) {

			// ? __element:element - 핀치 단일 요소
			// ? __data:object - data-pinchzoom의 데이터 객체값

			var $element = __element;
			var $pinch = mm.find(mm.selector(__data._classWrapper, '.'), $element)[0];
			// frame에서의 mm.scroll.el 는 window이고, 팝업에서의 mm.scroll.el는 mm_scroller 입니다.
			var $scroller = (mm.scroll.el === window) ? parent.mm.scroll.el : mm.scroll.el;

			var _isOnZoom = false;// ? :boolean - 줌 동작 구분
			var _isOnDrag = false;// ? :boolean - 드래그 동작 구분

			var zoom = { _scale: __data._scale, _beforeScale: __data._scale };
			var touch = {
				start: { _x: 0, _y: 0, distance: 0 },
				move: { _x: 0, _y: 0, _distance: 0 },
				between: { _x: 0, _y: 0 }// 두 터치점의 중간 값에 대한 x, y 좌표
			};
			...
			function translateMinmax() {
				...
				var _maxTop = $scroller.scrollTop - (_frameTop + $pinch.offsetTop) + __data.padding._top;// Y축 상단영역 끝까지 이동 가능한 translate
				var _maxBottom = _maxTop + parent.innerHeight - (_pinchHeight * zoom._scale) - __data.padding._bottom;// Y축 하단영역 끝까지 이동 가능한 translate
					...
					switch (__e.type) {
						case 'touchmove':
							...
							if (_isMultiTouch && !_isOnZoom) {
								...
								// __e.touches 좌표값은 화면 기준이기 때문에 mm_pinchzoom 영역이 디바이스 너비보다 작은경우
								// 터치의 좌표값을 보정하기 위해 $element.getBoundingClientRect()를 이용하여, 터치 위치를 보정
	...
	return {
		...
		update: function (__elements) {
			...
			// ? __elements:element - data-pinchzoom 속성이 있는 요소 또는 요소를 찾을 부모 요소
