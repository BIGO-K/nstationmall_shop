...
mm.link = function (__url, __option) {
	...
	switch (option._type) {
		...
		case 'link':
			mm.modal.close();
			// mm.loading.show();// ! 사파리 뒤로가기 이슈로 임시로 막음 >> 테스트 필요
...
mm.datepicker = (function () {
	...
	var base = {
			...
			if (!__data.__._isOpen) {
				...
				for (var _i = 1; _i <= _itemLength; _i++) {
					var $template = mm.element.create(mm.string.template([
						...
						'			<i class="mco_datepicker-select"></i>',
						'		</div>',
						'	</div>',
						'	<table>',
						'		<thead>',
			...
			_.forEach(mm.find('.mm__datepicker-item', $datepicker), function (__$item, __index) {
				...
				_.forEach(mm.find('.mm__datepicker__head-label', __$item), function (__$label, __index) {
					...
					for (var _i = _min; _i <= _max; _i++) {
						var $option = mm.element.create(mm.string.template('<option value="${VALUE}">${VALUE}</option>', { VALUE: String(_i).padStart(2, '0') }))[0];
						$select.append($option);

						if (_i === _year || _i === _month) {
							$option.selected = true;
							$select.previousElementSibling.textContent = String(_i).padStart(2, '0');
						}
					}
		...
		open: function (__$element) {
			...
			var position = mm.element.offset(__$element);
			mm.element.style($datepicker, { 'top': mm.number.unit(position.top + __$element.offsetHeight + document.documentElement.scrollTop), 'left': mm.number.unit(position.left + document.documentElement.scrollLeft) });

			data.__._isOpen = true;

			mm.event.on(window, 'mousedown', function closeDatepickerInlineHandler(__e) {

				if (__e.target.closest('.mm__datepicker-item')) return;
