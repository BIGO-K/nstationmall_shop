...
var ProdOption = function () {
	...
	var base = {
		...
		create: function () {
			...
			_.forEach(optionSubAll, function (__option) {
				...
				base.appendItem($fragSub.appendChild($optionItem.cloneNode(true)), Object.assign(mm.extend(__option), { _qty: 1 }));// 처음 생성할 때 품절 표시 안함
			...
			if (_hideOption !== null) {
				...
				// 버튼타입인 경우 레이어 오픈시 품절 옵션들은 품절처리로 전환
				if (_type === 'button') {
					_.forEach(mm.find('li', (_hideOption === 'main') ? $optionSub : $optionMain), function (__$li, __index) {

						var options = (_hideOption === 'main') ? optionSubAll : json.options;
						if (options[__index]._qty === 0) base.soldoutItem(__$li, false);

					});
				}

				mm.closest((_hideOption === 'main') ? $optionMain : $optionSub, _section)[0].classList.add('__option-hide');
			}
		...
		soldoutItem: function (__$element, __is) {
			...
			if (!data.$swap) {
				...
				data.$swap.append(mm.element.create(mm.string.template([
		...
		default: function (__isSet) {
			...
			_.forEach(json.options, function (__mainOption, __mainIndex) {
				...
				var $sections = mm.find((_type === 'select') ? '.mm_dropdown' : 'section', $ui);
					...
					if (json.selected._code === __subOption._code) {
						if (__subOption._qty > 0) {
							...
							// 선택
								...
								// 모든 옵션1에 대한 옵션2가 모두 같지 않기때문에 foreach의 arguments __subIndex와 실제 옵션의 순서간의 차이가 발생하여 옵션2의 index값을 별도로 구합니다.
	...
	return {
		...
		update: function (__json, __$ui) {
			...
			// 품절 옵션 열기
			var $layer = mm.find('.mm_layer', $ui)[0];
			var _classOpen = '__layer-open';

			// 옵션수량 변경
			mm.event.on(mm.find('.btn_option-change', $ui), 'click', function (__e) {

				__e.stopPropagation();

				var _isOpen = !mm.find(mm.selector(_classOpen, '.'), $ui)[0];
				if (_isOpen) {
					base.default();

					mm.class.remove(mm.find(mm.selector(_classOpen, '.')), _classOpen);
					$layer.classList.add(_classOpen);
				}
				else {
					var $sections = mm.find((_type === 'select') ? '.mm_dropdown' : 'section', $ui);
					...
					$layer.classList.remove(_classOpen);
				}

			});

			// 옵션변경 적용
			mm.event.on(mm.find('.btn_option-apply', $ui), 'click', function (__e) {
				...
				var $textOption = mm.find('.text_option', mm.siblings($ui, '.mm_product-item'))[0];
				$textOption.textContent = (_option.startsWith(' / ')) ? _option.replace(' / ', '') : _option;
				mm.find('strong', mm.siblings($ui, '.text_price'))[0].textContent = mm.number.comma(_price);

				if ($textOption.classList.contains('__option-soldout')) $textOption.classList.remove('__option-soldout');
			...
			var _itemHtml = mm.string.template((_type === 'button') ? [
				'<section>',
				'	<h6><b></b></h6>',
				'	<ul></ul>',
				'</section>'
			...
			var data = mm.data.get($ui).product;
			var $optionSelect = mm.find('.m_product-option-select', $layer)[0];

			if (_type === 'button') $optionSelect.classList.add('__select-button')

			// 옵션 영역 생성
			var $section = mm.find((_type === 'select') ? '.mm_dropdown' : 'section', $optionSelect)[0];
			if (!$section) $section = mm.element.create(_itemHtml)[0];

			var $fragment = document.createDocumentFragment();

			for (var _i = 0; _i < 2; _i++) {
				var $cloneItem = $fragment.appendChild($section.cloneNode(true));
				// var _text = mm.find('> .mm_strapline > b', $cloneItem)[0].textContent = mm.string.template('옵션${INDEX}', { INDEX: _i + 1 });
				mm.find('> h6 > b', $cloneItem)[0].textContent = mm.string.template('옵션${INDEX}', { INDEX: _i + 1 });

				// 셀렉트 타입
				if (_type === 'select') {
					mm.element.attribute($cloneItem, { 'data-dropdown': { _group: mm.string.template('dev_accrodion-option${INDEX}', { INDEX: data._index }) } });
					// mm.find('.btn_dropdown > b', $cloneItem)[0].textContent = _text;
				}
			}
...
mm.ready(function () {
	...
	mm.event.on(document, 'click', function (__e) {
		...
		if (!mm.find('.mm_layer.__layer-open')[0] || $target.closest('.mm_bom')) return;

		var $parent = mm.find('.mm_layer.__layer-open')[0].closest('.m_cart-option');
